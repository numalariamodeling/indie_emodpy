...
exp_id = '67187c60-f3e5-4fa9-92a7-11afdc357330'
...
run_size <- length(unique(SR$Run_Number))
for(rl in seq(5,25,5))
{
  rank_limit <- rl
  SR %>% 
...
SR_ranked %>%
  filter(rank==1) %>%
  left_join(SR, by=c("node","xTLH","cm_cov_u5")) %>%
  select(time, New_Clinical_Cases) %>%
  ...
  ggplot(aes(as.Date(month_year), y=cases)) +
  geom_point(size=3) +
  geom_line(size=1) +
  theme_minimal() +
  xlab("") + ylab("") +
