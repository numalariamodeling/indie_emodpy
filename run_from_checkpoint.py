...
checkpoint_id = '3e48d65c-391d-4916-89d4-a8bc774a78e1'   # checkpoint experiment from calibration

phase = 'burnin'                                         # burnin or pickup?
burnin_id = ''       # experiment id containing serialized population (required for pickup)


checkpoint_dir = "/".join((manifest.output_dir,tag,checkpoint_id))

checkpoint = pd.read_csv(os.path.join(checkpoint_dir,"checkpoint.csv"))

burnin_years = 37      # 1981-2017
pickup_years = 3       # 2018-2020
if(phase=="burnin"):
  num_seeds = 1         # number stochastic realizations
  rank_limit = 10      # include top # ranked parameter sets from checkpoint
if(phase=="pickup"): 
...
def build_camp(ranking=1):
    ...
    cp = checkpoint[checkpoint['rank']==ranking]
    cp = cp[['node','rank','xTLH','cm_cov_u5']]
    
    lh_scales = pd.DataFrame()
    nodes = []
    scales = []
    
    for n in cp['node']:
        node_cp = cp[cp['node']==n]
        nx = node_cp[['xTLH']]
        np = df[df['node_id']==n]
        np = np[['pop']]/1000
    ...
    lh_scales = pd.DataFrame({'NodeID': [nodes],
                             'CONSTANT': [scales],
    ...
    for n in cp['node']:
        ...
        if(phase == "burnin"):
            ...
            cm.add_treatment_seeking(camp, node_ids = [n],
            ...
            itn.add_itn_scheduled(camp, node_ids =[n],
            ...
            dc.add_drug_campaign(camp, node_ids=[n], campaign_type="MDA", drug_code="SPA", 
...
#############################
def update_campaign_single_parameter(simulation, ranking):
    """
    ...
    """
    build_campaign_partial = partial(build_camp, ranking = ranking)
...
def update_campaign_from_checkpoint(simulation, checkpoint, ranking):
    """
        This is a callback function that updates several parameters in the build_campaign function.
        the sweep is achieved by the itertools creating a an array of inputs with all the possible combinations
        see builder.add_sweep_definition(update_campaign_multiple_parameters function below
    ...
    cp = cp[cp[
    build_campaign_partial = partial(build_camp, cm_cov_u5)
...
def general_sim(selected_platform):
    ...
    if(phase=="burnin"):
        builder.add_sweep_definition(partial(update_campaign_single_parameter, ranking=[1,2,3,4,5]))
